...
console.log(req.currentUser)
    if(req.user.isAdmin){
        await RateCard.create(req.body).then((newRateCard)=>{
            newRateCard.uploadedBy = req.user.id
            newRateCard.uploadedDate = new Date()
    
            newRateCard.save().then(()=>{
...
router.put("/updatRateCard/:id",verify,async (req,res)=>{
        ...
        await RateCard.findByIdAndUpdate(req.params.id,{rateCardValidFrom,rateCardValidTo,activeStatus}).then((ratecard)=>{
            res.json(ratecard)
        }).catch((err)=>{
...
router.get("/allRateCards",verify,async (req,res)=>{
    ...
    await RateCard.find().populate([{path:'rateCardSubcon',select:'companyName'}]).then((ratecards)=>{
